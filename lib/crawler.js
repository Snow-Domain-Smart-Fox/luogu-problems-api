 ...
 * 说明：这是非原子操作（read-modify-write），在高并发场景下可能有竞态风险。
 * 若需要并发安全，请在 DB 侧增加 RPC（见建议）。
 */
 ...
 *
 * 增强点：
 * - 在创建 context/newPage 出现 browser closed 错误时尝试重建一次
 */
      ...
      if (crawled) {
        ...
        if (nextPage > total) {
          ...
          if (nt) {
            ...
            // 把 problems_crawled 的增量通过本文件 helper 展开并更新
